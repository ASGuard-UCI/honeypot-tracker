...
def _process_honeypot(honeypot, private_ip, connections, now):
    ...
    subprocess.run(["rsync", "-azv", f"root@{honeypot}:/root/ros-honeypot/data", f"/home/ubuntu/honeypot_tracker/data/{honeypot}/"])
    ...
    for file in files_to_process:
        for packet in rdpcap(file):
            ...
            not_yet_inserted = _has_been_inserted(cnx, timestamp, src_ip, honeypot, flag)

            if not_yet_inserted and ((private_ip is None and src_ip != honeypot) or (private_ip is not None and src_ip != private_ip)):
                ...
                data = (src_ip, datetime.strftime(timestamp, "%Y-%m-%d %H:%M:%S.%f"), honeypot, src_port, flag, payload, abuse_confidence_score, region, dst_port in (7400, 7401))
